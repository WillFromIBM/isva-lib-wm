...
var wm = {
  ...
  base64URLEncode: function(data) {
    resp = CryptoJS.enc.Base64.stringify(data).toString();
    resp = resp.replace(/=+$/, '');
    resp = resp.replace(/\+/g, '-');
    resp = resp.replace(/\//g, '_');
    return resp.toString();
  },
  generateJWT: function(header, payload, secret) {
    if (typeof header === 'object') {
      header = JSON.stringify(header);
    } else {
      if (typeof header != 'string') {
        IDMappingExtUtils.throwSTSException("generateJWT() called with invalid header type. Must be string or JSON Object");
      }
    }
    if (typeof payload === 'object') {
      ...
      if (typeof payload != 'string') {
        IDMappingExtUtils.throwSTSException("generateJWT() called with invalid payload type. Must be string or JSON Object");
      }
    }
    var H = wm.base64URLEncode(CryptoJS.enc.Utf8.parse(header));
    var P = wm.base64URLEncode(CryptoJS.enc.Utf8.parse(payload));
    var S = H + "." + P;
